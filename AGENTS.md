...
| **调研笔记** | `docs/research/` | **版本快照**：对某 DSH 版本的源码调研/问题调查，**开头必须写版本基准**（部署包版本；有源码对照仓再写路径），**文件名加入库日期前缀 `YYYY-MM-DD-`**（便于按时间浏览）。结论被新版本取代时**不追更**——移入 `docs/research/archived/` 并在文首标注被什么取代 |
