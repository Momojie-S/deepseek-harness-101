# MCP 配置（官方 dsh-mcp-client）

> 本篇只讲 **DSH 官方** `@deepseek-ai/dsh-mcp-client` 插件的全局/profile 级配置。每个外部 MCP server = 一个插件实例，配在 patch YAML 里，HMR 热生效。
>
> **项目级 MCP**（每个 workspace 自己的 `.dsh/mcp.servers.yml`，自研 `@momojie-s/dsh-workspace-mcp` 插件）的用法、配置、与全局同名遮蔽规则，见该插件仓 README：`plugins/dsh-workspace-mcp/`。
...
项目级 `workspace-mcp` 插件（自研）移植了同一套 supervisor 且补齐了上述盲区（`Session not found` 自愈），与全局 MCP 的同名遮蔽规则、headless 挂载法等见插件仓 README 与其 docs/design/。