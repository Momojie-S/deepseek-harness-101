...
> **因果澄清(2026-08-29 补)**:卡顿大头上文已归给事件洪流(阵发性,跟随子 agent 批次活跃度,实测高峰 242 帧/s vs 空闲 30 帧/s);本文的 session.list 规模化是**小头**(每次重连/刷新的一次性开销,664 条时约 1s);`sessions-archive/` 物理归档堆则**对性能零影响**(不进 `persistence.list`、不进任何接口,纯磁盘占用)。另注意"清完不卡"的体感是混杂变量:清理请求往往发生在批次收尾之后,洪流退潮才是主因。判断卡顿先测事件帧率(`.debug/temp/web-lag-probe/sse-probe.mjs`),再看会话数量。
>
> **补充测量(同日)**:①客户端**无轮询/聚焦刷新**,session.list 只在页面加载与 WS 重连时调用(重连罕见,WS 实测稳定 46-185 分钟)——列表体积的卡顿暴露面比预想更窄;②4 路并发 session.list 期间静态请求延迟 1-6ms→30-73ms,有干扰但非事件循环停摆级;③历史时间线反证:首次清理 664→160 后体感改善,此后列表稳定在 ~130 条,但"又开始卡了"仍反复出现——若列表体积是主因,130 条不应卡。**"清理后顺畅"的候选解释:清理后伴随的页面刷新重置了浏览器端累积状态(事件积压/长活标签页 DOM),与删除动作无关。判别实验:下次卡时先只刷新页面不清理——立刻顺畅则真凶是浏览器累积态;仍卡则测事件帧率。**
